...
export function Frame1() {
  ...
  return (
    <div style={{ width: "340px", backgroundImage: `url(${bg1})`, backgroundSize: "cover" }} className="px-2 border boder-1 rounded-4">
      <div style={{ fontSize: "50px", WebkitTextStrokeColor: "skyblue", WebkitTextStrokeWidth: "2px", textShadow: "10px 10px 10px black" }} className="fw-bolder text-warning" >{title}</div>
      ...
      {name && <div className="fw-bolder fs-1 text-info">{name}</div>}
      <div className="text-warning mb-3" style={{ fontSize: "small" }}>{message}</div>
    </div>
  )
}
