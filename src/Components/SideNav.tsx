...
export default function SideNav() {
  const { setname, settitle,
    setmessage, name, title, message, image1 } = useValues();
  return (
    <dialog className=" d-md-none" id="sidenav">
      <div
        style={{ width: "100vw", height: "100vh", }} className="d-flex justify-content-end">
        <div
          className="bg-dark d-flex flex-column overflow-hidden" style={{ width: "100%", maxWidth: "100%", borderLeft: "1px solid", }}>
          <div className="bg-info p-1 text-center">
            Edit Screen
          </div>
      ...
      <button className="btn-info btn fw-semibold" onClick={() => {
                (document.getElementById("sidenav") as HTMLDialogElement).close();
            }}>Done</button>
          </div>
        </div>
      </div>
